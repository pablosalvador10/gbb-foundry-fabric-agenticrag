...
def verify_integration():
    ...
    try:
        ...
        uploaded = client.datasets.upload_file(
            name=test_dataset_name, version="1.0", file_path=temp_file.name
        )
    ...
    print(
        f"4. Look for datasets with names starting with 'evaluation_dataset_' or 'verification_test_'"
    )

    print(f"\n🎯 YOUR DATA IS SUCCESSFULLY INTEGRATED WITH AI FOUNDRY!")
    print(
        "The evaluation runs will also sync to the portal when the API becomes fully available."
    )
