...
This module implements an intelligent orchestrator that uses Azure OpenAI
to make semantic routing decisions between specialized agents in the enterprise system.
"""
...
logger = get_logger("agent_registry.azure_openai.intelligent_orchestrator")
...
- "Which airlines have the most flights?" → fabric_agent""",
            ...
            logger.info("Intelligent routing agent created successfully")

        except Exception as e:
            logger.error(f"Failed to create routing agent: {str(e)}")
            self.routing_agent = None
    ...
    async def route_query(self, query: str) -> Tuple[str, str, Dict[str, Any]]:
        ...
        if not self.routing_agent:
            return (
                "fabric_agent",
                "Routing agent not available, defaulting to fabric agent",
                {
                    "agent": "fabric_agent",
        ...
        try:
            ...
            return (
                agent_name,
                f"Semantic analysis routed to {routing_info['name']}",
                routing_info,
            )
            ...
            return (
                "fabric_agent",
                f"Routing error, defaulting to fabric agent: {str(e)}",
                {
                    "agent": "unified_fabric",
                    "name": "Fabric Data Agent",
                    "icon": "📊",
    ...
    async def execute_query(self, query: str) -> Tuple[str, Dict[str, Any]]:
        ...
        This method performs intelligent routing to select the optimal agent and
        executes the query, handling both synchronous and asynchronous agent implementations.

        :param query: The user's question or request
        :return: Tuple containing (response_text, routing_info)
        ...
        try:
            ...
            if agent_name == "foundry_agent" and self.foundry_agent:
                # ChatAgent.run() returns a coroutine, so we need to await it
            ...
            logger.error(f"Error executing query with {agent_name}: {str(e)}")
...
async def intelligent_agent_routing(
    query: str, foundry_agent: Any, fabric_agent: Any
