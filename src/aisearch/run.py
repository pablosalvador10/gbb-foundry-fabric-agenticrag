...
class PolicyIndexingPipeline:
    ...
    def __init__(self, config_path: str = "src/azureAIsearch/settings.yaml"):
        """
    ...
    def upload_documents(self, local_path: str) -> None:
        ...
        """
        try:
            logger.info(f"Starting document upload from local path: {local_path}")
            container_client = self.blob_service_client.get_container_client(
                self.blob_container_name
            )
            logger.info(f"Connected to Blob container: {self.blob_container_name}")
            ...
            logger.error(f"Failed to upload documents from {local_path}: {e}")
            raise
